...
summary(lr_model)
lr=lm(Count~.,train)
y_pred=predict(lr_model,test[,1:12])
#Summary of Linear model
summary(lr_model)
r=sum((y_pred-test[,13])^2)/sum((test[,13]-mean(test[,13]))^2)
1-r #-> 0.792
...
summary(RF_model)
y_pred_rf = predict(RF_model, test[,-13])
...
predict_y=function(model,df,type){
  ...
  print('Sample Input')
  print(df[3,])
  X=subset(df,select = -c(Instant,Date,Registered,Casual,Temp,Holiday))
